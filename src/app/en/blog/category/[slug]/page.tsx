...
export async function generateMetadata({
  ...
  const all = await getAllBlogSummaries("en");
  const postCount = all.filter((p) => p.category.toLowerCase() === cat).length;
  const items = postCount === 1 ? "article" : "articles";
  const description = `${postCount} ${items} on ${label.toLowerCase()} from the Maroc 360 team — frameworks, playbooks, and practical guides for marketers in Morocco.`;
  return {
    title: `${label} – Blog | ${siteConfig.name}`,
    description,
    alternates: { canonical: `${siteConfig.url}/en/blog/category/${cat}` },
  };
}
...
export default async function BlogCategoryPage({ params }: PageProps) {
      ...
      <>
        ...
        <main id="main" tabIndex={-1} className="container-page section-y text-center">
          ...
          <p className="body-lg mt-4">
            <Link href="/en/blog" className="link-underline font-semibold">
  ...
  const pageUrl = absoluteUrl(`/en/blog/category/${cat}`);
  const schema = webPageSchema({
    id: `${pageUrl}#webpage`,
    name: `${label} – Blog | ${siteConfig.name}`,
    description: `${posts.length} ${posts.length === 1 ? "article" : "articles"} on ${label.toLowerCase()} from the Maroc 360 team.`,
    url: pageUrl,
    inLanguage: "en",
    type: "CollectionPage",
  });
          ...
          <div className="container-page">
            {posts.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {posts.map((post) => (
                  <PostCard key={post.href} post={post} dict={dict.blogPost} />
                ))}
              </div>
            ) : (
              <div className="text-center py-12">
                <p className="text-text-secondary">
                  ...
                  <Link href="/en/blog" className="link-underline font-semibold text-text-primary">
                    {dict.blogPost.categoryBrowseAll}
                  </Link>
                </p>
              </div>
