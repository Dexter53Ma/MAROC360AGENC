...
You are about to audit the website at **the target URL provided by the user** for SEO, technical, content, performance, security, JavaScript rendering, and AI readiness using the SEOmator CLI.
...
1. **Verify CLI is installed.** Run `seomator self doctor`. If it reports "seomator: command not found", install it first:
   ...
   ```
2. **Parse `the target URL provided by the user` for the target URL and flags.** The first argument should be a full URL (including protocol). If none was provided, ask the user which URL to audit — do not guess.
